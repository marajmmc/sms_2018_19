                ...
                <table class="table table-bordered">
                    ...
                    <tbody id="items_container">
                        ...
                        foreach($items as $index=>$value)
                        {
                            ...
                            ?>
                            <tr id="id_tr_<?php echo $index+1;?>" class="<?php echo $class_bg_warning?>">
                                <td>
                                ...
                                <td class="text-right">
                                    <label class="control-label stock_available_pkt" id="stock_available_pkt_<?php echo $index+1;?>">
                                        <?php
                                        echo System_helper::get_string_quantity($too_variety_info[$value['variety_id']][$value['pack_size_id']]['stock_available_pkt']);
                                        ?>
                                    </label>
                                </td>
                                <td class="text-right">
                                    <label class="control-label stock_available_kg" id="stock_available_kg_<?php echo $index+1;?>">
                                        <?php
                                        echo System_helper::get_string_kg($too_variety_info[$value['variety_id']][$value['pack_size_id']]['stock_available_kg']);
                                        ?>
                                    </label>
                                </td>
                                <td class="text-right">
                                    <label class="control-label" id="quantity_request_<?php echo $index+1;?>" data-current-id="<?php echo $index+1;?>"><?php echo System_helper::get_string_quantity($value['quantity_request']); ?></label>
                                </td>
                                <td class="text-right">
                                    <label class="control-label" id="quantity_request_kg_<?php echo $index+1;?>"> <?php echo System_helper::get_string_kg($quantity_request_kg);?> </label>
                                </td>
                                <td>
                                ...
                                <td class="text-right">
                                    <label class="control-label" id="quantity_approve_kg_<?php echo $index+1;?>"> <?php echo System_helper::get_string_kg($quantity_approve_kg);?> </label>
                                </td>
                                <td>
...
<script>
    ...
    {
        ?>
        var tow_variety_info=JSON.parse('<?php echo json_encode($too_variety_info);?>');
        <?php
    }
    else
    {
    ...
    function calculate_total()
    {
        ...
        $('#quantity_total_approve_kg').html(get_string_kg(quantity_total_approve_kg));
    }
    $(document).ready(function()
    {
        ...
        $(document).on("change","#items_container .pack_size_id",function()
        {
            ...
            if(tow_variety_info[variety_id][pack_size_id]!==undefined)
            {
                ...
                $("#stock_available_kg_"+current_id).html(get_string_kg(tow_variety_info[variety_id][pack_size_id]['stock_available_kg']));
        ...
        $(document).on('input','#items_container .quantity_approve',function()
        {
            ...
            $("#id_tr_"+current_id).removeClass('bg-danger');
            if(quantity_approve>stock_available_pkt)
            {
                $(this).addClass('bg-danger');
                $("#id_tr_"+current_id).addClass('bg-danger');
            }

            /*$("#stock_available_kg_"+current_id).removeClass('bg-danger');
            if(quantity_approve_kg>stock_available_kg)
            {
                $("#stock_available_kg_"+current_id).addClass('bg-danger');
            }*/

            $("#quantity_approve_kg_"+current_id).html(get_string_kg(quantity_approve_kg));
            calculate_total();
        });
    });
