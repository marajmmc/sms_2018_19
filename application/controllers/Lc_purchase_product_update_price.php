...
class Lc_purchase_product_update_price extends Root_Controller
{
    ...
    private function system_list()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Pending List :: Setup Purchase Product Update Price";
            $ajax['status']=true;
    ...
    private function system_list_all()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="All List :: Setup Purchase Product Update Price";
            $ajax['status']=true;
    ...
    private function system_get_items_all()
    {
        ...
        $this->db->where('lc.status_open !=',$this->config->item('system_status_delete'));
        $this->db->where('lc.status_receive',$this->config->item('system_status_complete'));
        $this->db->where('lc.status_average_receive',$this->config->item('system_status_complete'));
        $this->db->order_by('lc.id','DESC');
    ...
    private function system_edit_price($id)
    {
        if((isset($this->permissions['action2']) && ($this->permissions['action2']==1)))
        {
            ...
            $ajax['status']=true;
            $ajax['system_content'][]=array("id"=>"#system_content","html"=>$this->load->view($this->controller_url."/edit_price",$data,true));
            if($this->message)
            {
                $ajax['system_message']=$this->message;
            }
            $ajax['system_page_url']=site_url($this->controller_url.'/index/edit_price/'.$item_id);
            $this->json_return($ajax);
        }
        else
        {
