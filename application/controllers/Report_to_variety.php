...
class Report_to_variety extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    private function system_search()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="TO Variety Wise Report Search";
            $ajax['status']=true;
    ...
    private function system_list()
    {
        ...
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            elseif($reports['report_name']=='to_wise')
            {
                $method='list_to_wise';
                $data['system_preference_items']= System_helper::get_preference($user->user_id,$this->controller_url,$method,$this->get_preference_headers($method));
                $data['title']="TO Wise Report";
                $ajax['system_content'][]=array("id"=>"#system_report_container","html"=>$this->load->view($this->controller_url."/list_to_wise",$data,true));
            }
            else
            {
    ...
    private function system_get_items_to()
    {
        ...
        /*$this->db->select('SUM(CASE WHEN transfer_wo.'.$date_type.'>='.$date_start.' and transfer_wo.'.$date_type.'<='.$date_end.' then details.quantity_request ELSE 0 END) quantity_request',false);
        $this->db->select('SUM(CASE WHEN transfer_wo.'.$date_type.'>='.$date_start.' and transfer_wo.'.$date_type.'<='.$date_end.' then details.quantity_approve ELSE 0 END) quantity_approve',false);
        $this->db->select('SUM(CASE WHEN transfer_wo.'.$date_type.'>='.$date_start.' and transfer_wo.'.$date_type.'<='.$date_end.' then details.quantity_receive ELSE 0 END) quantity_receive',false);*/
