...
class Stock_in_raw_foil extends Root_Controller
{
    ...
    private function system_get_items()
    {
        ...
        foreach($items as &$item)
        {
            ...
            $item['quantity_total_kg']=number_format($item['quantity'],3,'.','');
        }
    ...
    private function system_details($id)
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $this->db->from($this->config->item('table_sms_stock_in_raw_foil').' raw_foil');
            ...
            $data['title']="Stock In (Common Foil) Details :: ".Barcode_helper::get_barcode_raw_foil_stock_in($item_id);
            $ajax['status']=true;
    ...
    private function system_details_print($id)
    {
        if((isset($this->permissions['action4']) && ($this->permissions['action4']==1)))
        {
            ...
            $data['title']="Stock In (Common Foil) Print :: ".Barcode_helper::get_barcode_raw_foil_stock_in($item_id);
            $ajax['status']=true;
