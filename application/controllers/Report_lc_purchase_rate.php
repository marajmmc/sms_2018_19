...
class Report_lc_purchase_rate extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    {
        $this->lang->language['LABEL_RATE_WEIGHTED_RECEIVE']="Unit Price Rate Receive";
        $this->lang->language['LABEL_RATE_WEIGHTED_COMPLETE']="Unit Price Rate Complete";
        $this->lang->language['LABEL_AMOUNT_TOTAL_RATE_RECEIVE']="Total Price Rate Receive";
        $this->lang->language['LABEL_AMOUNT_TOTAL_RATE_COMPLETE']="Total Price  Rate Complete";
    }
    ...
    private function get_preference_headers()
    {
        ...
        $data['rate_weighted_receive']= 1;
        $data['amount_total_rate_receive']= 1;
        $data['rate_weighted_complete']= 1;
        $data['amount_total_rate_complete']= 1;
        return $data;
    }
    private function system_search()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Variety Wise Purchase Rate Search";
            $ajax['status']=true;
    ...
    private function system_list()
    {
        ...
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Variety Wise Purchase Rate Report";
            $ajax['status']=true;
    ...
    private function system_get_items()
    {
        ...
        $lc_purchase_variety_rates=$this->get_lc_purchase_variety_rates($date_end,$variety_ids);
    ...
    private function get_lc_purchase_variety_rates($date_receive,$variety_ids)
    {
        ...
        foreach($results as $result)
        {
            $lc_detail_ids[$result['lc_details_id']]=$result['lc_details_id'];
        }
        $this->db->from($this->config->item('table_sms_lc_details') . ' details');
