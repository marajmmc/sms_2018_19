...
class Report_to_to extends Root_Controller
{
    ...
    private function system_get_items()
    {
        ...
        {
            $to_all[$result['outlet_id']][$result['id']]=$result;
        }
        foreach($outlets as $result)
        {
            if(isset($to_all[$result['outlet_id']]))
            {
                $first_row=true;
                foreach($to_all[$result['outlet_id']] as $to_id=>$to)
                {
                    $item=array();
                    ...
                    $item['status_request']=$to['status_request'];
                    $item['date_approve']=System_helper::display_date($to['date_approve']);
                    $item['quantity_total_approve']=number_format($to['quantity_total_approve_kg'],3,'.','');
                    $item['status_approve']=$to['status_approve'];
                    $item['date_delivery']=System_helper::display_date($to['date_delivery']);
                    $item['status_delivery']=$to['status_delivery'];
                    $item['date_receive']=System_helper::display_date($to['date_receive']);
                    $item['quantity_total_receive']=number_format($to['quantity_total_receive_kg'],3,'.','');
                    $item['status_receive']=$to['status_receive'];
                    $item['status_receive_forward']=$to['status_receive_forward'];
                    $item['status_receive_approve']=$to['status_receive_approve'];
                    $item['status_system_delivery_receive']=$to['status_system_delivery_receive'];
                    if($to['status_approve']==$this->config->item('system_status_rejected'))
                    {
                    ...
                    if($to['status_system_delivery_receive']==$this->config->item('system_status_yes'))
                    {
                        $item['status_receive_forward']='';
                        $item['status_receive_approve']='';
                    }
