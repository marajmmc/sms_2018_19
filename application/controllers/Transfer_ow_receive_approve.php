...
class Transfer_ow_receive_approve extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Transfer_ow_receive_approve');
        $this->controller_url='transfer_ow_receive_approve';
    ...
    private function system_list()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Outlet to HQ Receive Approve List";
            $ajax['status']=true;
    ...
    private function system_list_all()
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Outlet to HQ Transfer Receive Approve All List";
            $ajax['status']=true;
    ...
    private function system_save()
    {
        ...
        if($id>0)
        {
            ...
            if(!$this->check_my_editable($result['item']))
            {
                System_helper::invalid_try('details',$id,'User location Not Assign.');
                $ajax['status']=false;
                $ajax['system_message']=$this->lang->line("YOU_DONT_HAVE_ACCESS");
                $this->json_return($ajax);
            }
        ...
        Query_helper::add($this->config->item('table_sms_transfer_ow_receive_solves'),$data);
    ...
    private function system_details($id)
    {
        if(isset($this->permissions['action0'])&&($this->permissions['action0']==1))
        {
            ...
            $data['title']="Outlet to HQ Transfer Receive Approve Details :: ". Barcode_helper::get_barcode_transfer_warehouse_to_outlet($data['item']['id']);
            $ajax['status']=true;
