...
class Transfer_wo_approve extends Root_Controller
{
    ...
    private function system_save_forward()
    {
        ...
        if($id>0)
        {
            ...
            {
                $ajax['status']=false;
                $ajax['system_message']='Approved is required.';
                $this->json_return($ajax);
            }
            ...
            if(!$data['item'])
            {
                System_helper::invalid_try('Update Approved Non Exists',$id);
                $ajax['status']=false;
                $ajax['system_message']='Invalid Try.';
                $this->json_return($ajax);
            }
            ...
            if(!$this->check_my_editable($data['item']))
            {
                System_helper::invalid_try('Edit Approved Permission Non Exists',$id);
                $ajax['status']=false;
                $ajax['system_message']=$this->lang->line("YOU_DONT_HAVE_ACCESS");
                $this->json_return($ajax);
            }
