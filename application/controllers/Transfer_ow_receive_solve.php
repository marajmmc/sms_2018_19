...
class Transfer_ow_receive_solve extends Root_Controller
{
    public $message;
    public $permissions;
    ...
    public function __construct()
    {
        parent::__construct();
        $this->message="";
        $this->permissions=User_helper::get_permission('Transfer_ow_receive_solve');
        $this->controller_url='transfer_ow_receive_solve';
    }
    ...
    private function system_edit($id)
    {
        if(isset($this->permissions['action2'])&&($this->permissions['action2']==1))
        {
            ...
            $this->db->select(
                ...
                transfer_ow.remarks_receive_approve
                ');
            $this->db->join($this->config->item('table_login_csetup_cus_info').' outlet_info','outlet_info.customer_id=transfer_ow.outlet_id AND outlet_info.type="'.$this->config->item('system_customer_type_outlet_id').'"','INNER');
    ...
    private function system_save()
    {
        ...
        if($id>0)
        {
            ...
            $result=Query_helper::get_info($this->config->item('table_sms_transfer_ow_receive_solves'),'*',array('id ='.$id),1);
            if(!$result)
            {
        ...
        $path='images/transfer/ow_receive_solve/'.$id;
//        $dir=(FCPATH).$path;
//        if(!is_dir($dir))
//        {
//            mkdir($dir, 0777);
//        }
        ...
        Query_helper::update($this->config->item('table_sms_transfer_ow_receive_solves'),$item,array('id='.$id));
